...
var defaultOptions = function(context) {
  console.log('get default options for ctx', context)
  ...
  if (data.imageCollection) {
    ...
    console.log('got file collection: ', imageCollection);
    options.onImageUpload = function(files, editor, $editable) {
      ...
      imageCollection.insert(file, function(err, fileObj) {
        if (!err) {
          console.log('upload successful, context', context)
...
var insertImage = function(context, url) {
  ...
  console.log('insert image', element, 'ctx', context)
...
Template.afSummernote.onCreated( function() {
  ...
  if (imageOptions.imageCollection) {
    ...
    this.autorun(function() {
      ...
      if (newImage) {
        var url;
        var bucket = imageOptions.s3Bucket || Summernote.s3Bucket;
        if (bucket) {
          var storeName = imageOptions.storeName || Summernote.s3Bucket || imageOptions.imageCollection;
          var folder = imageOptions.s3subFolder || Summernote.s3subFolder;
